...
void merge(vector<int>& arr, int first, int middle, int last){
    ...
    //now the temp contains the sorted stuff that is to be placed in arr from index first to last
    for(int z = first; z<=last;z++){
        arr[z] = temp[z-first];
    }
}
...
int main(){
    ...
    node* head = new node(-1);
    node* mover = head;
    cout<<endl<<"Enter the nodes of the linked list : ";
    int n=-1;
    do{
        cin>>n;
        if(n!=-1){
    ...
    head = head ->next;
    mover = head;
    cout<<endl<<"The formed linked list before sorting : ";
    while(mover!=nullptr){
        cout<<mover->val<<" ";
        mover = mover ->next;
    }
...
int main(){
    ...
    cout<<endl<<"The BFS traversal : ";
    bfs_traversal(adj,x);

    vector<int> visited(n+1,0);
    cout<<endl<<"The DFS traversal : ";
    dfs_traversal(adj,x,visited);
